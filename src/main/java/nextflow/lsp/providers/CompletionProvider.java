...
import nextflow.lsp.compiler.util.GroovyASTUtils;
...
public class CompletionProvider {
    ...
    private static final CompletionItem NXF_DIRECTIVES_ECHO = new CompletionItem("echo");
    private static final CompletionItem[] NXF_DIRECTIVES = new CompletionItem[]{
            new CompletionItem("accelerator"), new CompletionItem("afterScript"), new CompletionItem("beforeScript"), new CompletionItem("cache"),
            new CompletionItem("conda"), new CompletionItem("container"), new CompletionItem("containerOptions"), new CompletionItem("cpus"), new CompletionItem("clusterOptions"),
            new CompletionItem("disk"), NXF_DIRECTIVES_ECHO, new CompletionItem("errorStrategy"), new CompletionItem("executor"), new CompletionItem("ext"), new CompletionItem("machineType"),
            new CompletionItem("maxErrors"), new CompletionItem("maxForks"), new CompletionItem("maxRetries"), new CompletionItem("memory"), new CompletionItem("module"), new CompletionItem("penv"),
            new CompletionItem("pod"), new CompletionItem("publishDir"), new CompletionItem("queue"), new CompletionItem("label"), new CompletionItem("scratch"), new CompletionItem("storeDir"),
            new CompletionItem("stageInMode"), new CompletionItem("stageOutMode"), new CompletionItem("tag"), new CompletionItem("time"), new CompletionItem("validExitStatus")
    };

    static {
        NXF_DIRECTIVES_ECHO.setInsertText("echo true");
        NXF_DIRECTIVES_ECHO.setDocumentation(new MarkupContent(MarkupKind.MARKDOWN,
    ...
    private static final CompletionItem[] NXF_INPUTS = new CompletionItem[]{
            NXF_INPUTS_VAL, new CompletionItem("env"), new CompletionItem("file"), new CompletionItem("path"), new CompletionItem("stdin"), new CompletionItem("tuple"), new CompletionItem("each")
    };

    static {
        NXF_INPUTS_VAL.setInsertText("val ${1:var_name}");
        NXF_INPUTS_VAL.setDocumentation("Lets you access the received input value by its name in the process script");
    }

    private static final CompletionItem[] NXF_OUTPUTS = new CompletionItem[]{
            NXF_INPUTS_VAL, new CompletionItem("file"), new CompletionItem("path"), new CompletionItem("env"), new CompletionItem("stdout"), new CompletionItem("tuple")
    };
    ...
    static {
        ...
        NXF_PROCESS.setInsertText("process ${1:MY_PROCESS} {\n" +
                "\tinput:\n" +
                "\t${2|" + String.join(",", Arrays.asList(NXF_INPUTS).stream().map(CompletionItem::getLabel).collect(Collectors.toList())) + "|} ${3:var_name}\n\n" +
                "\toutput:\n" +
                "\t${4|" + String.join(",", Arrays.asList(NXF_OUTPUTS).stream().map(CompletionItem::getLabel).collect(Collectors.toList())) + "|} ${5:var_name}\n\n" +
                "\tscript:\n" +
                "\t\"\"\"\n" +
                "\t${6:MY_SCRIPT}\n" +
                "\t\"\"\"\n" +
    ...
    public CompletableFuture<Either<List<CompletionItem>, CompletionList>> provideCompletion(TextDocumentIdentifier textDocument, Position position, CompletionContext context) {
        if (ast == null) {
    ...
    private void populateItemsFromPropertyExpression(PropertyExpression propExpr, Position position, List<CompletionItem> items) {
    ...
    private void populateItemsFromMethodCallExpression(MethodCallExpression methodCallExpr, Position position, List<CompletionItem> items) {
    ...
    private void populateItemsFromConstructorCallExpression(ConstructorCallExpression constructorCallExpr, Position position, List<CompletionItem> items) {
    ...
    private void populateProcess(CompletionItem[] values, String memberNamePrefix, Set<String> existingNames, List<CompletionItem> items) {
        List<CompletionItem> variableItems = Arrays.asList(values).stream().filter(ci -> {
            if (ci.getLabel().startsWith(memberNamePrefix) && !existingNames.contains(ci.getLabel())) {
                existingNames.add(ci.getLabel());
                return true;
            }
            return false;
        }).map(ci -> {
    ...
    private void populateItemsFromScope(ASTNode node, String namePrefix, List<CompletionItem> items) {
        ...
        while (current != null) {
            if (current instanceof ClassNode) {
                ...
                if (!inProcess) {
                    processLabel = findProcessLabel(node, block);
                }
            } else if (current instanceof MethodCallExpression) {
                MethodCallExpression methodCallExpression = (MethodCallExpression) current;
                inProcess = "process".equals(methodCallExpression.getMethodAsString());
            }
            current = ast.getParent(current);
        }

        if (inProcess && processLabel != null && node instanceof VariableExpression) {
            if (processLabel.equals("directives")) {
                populateProcess(NXF_DIRECTIVES, namePrefix, existingNames, items);
            } else if (processLabel.equals("input")) {
                populateProcess(NXF_INPUTS, namePrefix, existingNames, items);
            } else if (processLabel.equals("output")) {
                populateProcess(NXF_OUTPUTS, namePrefix, existingNames, items);
            }
        }
    ...
    private String findProcessLabel(ASTNode node, BlockStatement block) {
        ...
        for (Statement st : block.getStatements()) {
            if (st.getStatementLabel() != null) {
                result = st.getStatementLabel();
            }
            if (st instanceof ExpressionStatement) {
                ExpressionStatement est = (ExpressionStatement) st;
                if (est.getExpression() == node) {
                    return result;
                }
            }
        }
    ...
    private void populateTypes(ASTNode offsetNode, String namePrefix, Set<String> existingNames, boolean includeClasses, boolean includeInterfaces, boolean includeEnums, List<CompletionItem> items) {
